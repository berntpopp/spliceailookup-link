...
- **CI tests the interpreter that actually ships.** The `quality` job now runs
  ...
  coverage gate still runs once (on 3.14, push to `main`).
...
Dependency-supply-chain maintenance. This repository had **no
`.github/dependabot.yml`**, so its Python dependencies, GitHub Actions and Docker
base image had never been auto-bumped — the "0 open Dependabot PRs" signal was the
absence of a watcher, not health. This release establishes the watcher and sweeps
the drift it had been hiding.

### Added

- `.github/dependabot.yml` — fleet-standard coverage for four ecosystems: `uv` at
  `/`, `github-actions` at `/`, and `docker` + `docker-compose` at `/docker`. Weekly
  Monday, Europe/Berlin, staggered 04:00/04:15/04:30/04:45, limit 5 PRs each,
  commit-message prefixes `deps` (uv/docker) and `ci` (actions).

### Changed

- Refreshed the whole lock (`uv lock --upgrade`): 39 packages moved. Notable direct
  dependencies — `fastapi` 0.136.3 → 0.141.1, `uvicorn` 0.49.0 → 0.52.0,
  `mcp` 1.28.1 → 1.29.0, `fastmcp` 3.4.4 → 3.4.5, `typer` 0.26.7 → 0.27.0,
  `mypy` 2.1.0 → 2.3.0, `pytest` 9.0.3 → 9.1.1, `ruff` 0.15.16 → 0.16.0.
- Raised `mcp[cli]` floor to `>=1.28.1`. The lock already pinned a fixed `mcp`, but
  the old `>=1.27.0` floor still *permitted* a fresh resolve to land on a release
  vulnerable to GHSA "WebSocket server transport does not support Host/Origin
  validation". The floor now makes that resolve impossible.
- Raised `fastmcp[tasks]` floor to `>=3.4.5` and `pytest` floor to `>=9.1.1`,
  keeping the repo's floor-tracks-lock convention for those pins.
- Ruff config now uses `select` instead of `extend-select` with the identical rule
  list. Ruff 0.16.0 grew its implicit default rule set from 59 to 413 rules, so
  `extend-select` would have silently inherited ~350 unopted rules. The pinned list
  already supersets ruff's pre-0.16 default (E4/E7/E9 + F), so enforced lint policy
  is byte-identical to what this repo has always enforced.
- Docker base image re-pinned to the current `python:3.14-slim` digest
  (`sha256:cea0e604…`, was `sha256:b877e50b…`).
- GitHub Actions bumped and re-pinned by SHA: `actions/checkout` v7.0.0 → v7.0.1
  (and the stale v6.0.3 pin in `container-security.yml` brought onto the same
  version as the rest of the fleet), `actions/setup-python` v6 → v7.0.0,
  `astral-sh/setup-uv` v8.2.0 → v9.0.0.
- Synced the generated `CITATION.cff` `version:` line (3.0.8 → 4.0.1); the file is
  otherwise centrally regenerated by genefoundry-router and is not hand-edited.
