...
| Observability | 9 | The prediction responses observed in this assessment had `request_id`, `timing.elapsed_ms`, `upstream_elapsed_ms`, `cache` (hit/miss/partial + `cache_age_s` + `cache_ttl_s`), and a `capabilities_version` content hash so a warm client can detect drift and skip re-fetching. |
