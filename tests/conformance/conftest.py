"""Bind the canonical HTTP-policy v1 suite to SpliceAI's real guard helpers."""
...
class _Adapter:
    ...
    def allow(self, url: str) -> object:
        self._run(self._guard(httpx.Request("GET", url)))
        return "allowed"
    ...
    def read_decoded(self, chunks: Iterable[bytes], cap: int) -> None:
        ...
        async def read() -> None:
            async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
                await read_capped(client, "GET", "https://allowed.example/", max_bytes=cap)
