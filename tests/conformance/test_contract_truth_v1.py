"""Contract Truth v1 gate against the live SpliceAI Lookup MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    upstream_accesses: list[tuple[str, str]] = []

    class OfflineUpstream:
        def __init__(self, name: str) -> None:
            self.name = name

        def __getattr__(self, attribute: str) -> Never:
            upstream_accesses.append((self.name, attribute))
            raise AssertionError(
                f"contract discovery accessed the offline {self.name} service: {attribute}"
            )

    service = SpliceService(
        scoring_client=cast(ScoringClient, OfflineUpstream("scoring")),
        ensembl_client=cast(EnsemblVepClient, OfflineUpstream("VEP")),
    )
    mcp = create_spliceai_mcp(service_factory=lambda: service)

    tools = await mcp.list_tools()
    assert outbound_attempts == []
    assert upstream_accesses == []
